...
const heap_isEmpty_Queue = (queue_address) => (heap_get(queue_address + 1) === -1);
// Channel
// [1 byte tag, 4 bytes unused,
//  2 bytes #children, 1 byte unused]
...
const builtin_object = {
    ...
    Wait: () => {
        ...
        if (!is_WaitGroup(waitGroup_address))
...
const microcode = {
    ...
    SEND: (instr) => {
        ...
        if (!is_Chan(address))
            error("can only send to a channel");
    ...
    REC: (instr) => {
        ...
        const address = heap_get_Environment_value(E, instr.pos);
        if (!is_Chan(address))
            error("can only receive from a channel");
        const qcount = heap_get(address + 1);
        const dataqsiz = heap_get(address + 2);
        const buffer_addr = heap_get(address + 3);
        const sendx = heap_get(address + 4);
